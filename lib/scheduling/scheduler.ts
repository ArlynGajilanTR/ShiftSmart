...
export function autoAssignShifts(
  ...
  for (const shift of shifts) {
    ...
    const availableUsers = users.filter(
      (u) => !assignedToShift.some((a) => a.user_id === u.id) && u.bureau_id === shift.bureau_id
    );
    ...
    const stillNeeded =
      ...
      (assignedToShift.length + newAssignments.filter((a) => a.shift_id === shift.id).length);
    const remainingUsers = availableUsers.filter(
      (u) => !newAssignments.some((a) => a.user_id === u.id)
    );
