...
export async function callClaude(
  ...
  try {
    ...
    if (retryCount > 0) {
      console.log(
        `[Retry] Attempt ${retryCount}/${MAX_RETRIES} after ${BASE_DELAY_MS * Math.pow(2, retryCount - 1)}ms delay`
      );
    }
    ...
    console.log(
      `[AI Performance] Generated ${fullText.length} chars in ${elapsed}ms (~${tokenCount} tokens)`
    );
    ...
    if (isRetryable && retryCount < MAX_RETRIES) {
      ...
      console.log(
        `[Retry] Retryable error detected, waiting ${delay}ms before retry ${retryCount + 1}/${MAX_RETRIES}`
      );
      await sleep(delay);
